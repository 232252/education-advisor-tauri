// =============================================================
// CDP 深度交互测试 — Electron 版
// 连接到运行中的 Electron 应用 (CDP 9222), 通过 window.api 测试所有功能
// =============================================================
...
async function callApi(page, apiPath, ...args) {
  return await page.evaluate(async ({ path, args }) => {
    try {
      ...
      if (typeof fn !== 'function') {
        return { ok: false, error: `window.api.${path} is not a function` }
      }
...
async function main() {
  ...
  console.log('╔══════════════════════════════════════════════════╗')
  console.log('║  CDP 深度交互测试 — Electron window.api 版        ║')
  console.log('╚══════════════════════════════════════════════════╝\n')
  ...
  const listRes = await callApi(page, 'eaa.listStudents')
  log('学生列表加载', listRes.ok && listRes.data?.success !== false, listRes.error || '')
  const initialCount = listRes.data?.data?.students?.length || 0

  // 添加测试学生
  const addRes = await callApi(page, 'eaa.addStudent', 'CDP_Electron_A')
  log('添加测试学生A', addRes.ok && addRes.data?.success !== false, addRes.error || '')

  // 重复添加(应该幂等或返回失败)
  const addDupRes = await callApi(page, 'eaa.addStudent', 'CDP_Electron_A')
  log('重复添加学生处理', addDupRes.ok, '(幂等或失败均算通过)')

  // 空名字拒绝
  const addEmptyRes = await callApi(page, 'eaa.addStudent', '')
  log('空名字拒绝', !addEmptyRes.data?.success, '(应返回失败)')

  // 参数注入拒绝
  const addInjectRes = await callApi(page, 'eaa.addStudent', 'test; rm -rf /')
  log('参数注入拒绝', !addInjectRes.data?.success, '(应返回失败)')

  // 超长名字拒绝
  const addLongRes = await callApi(page, 'eaa.addStudent', 'A'.repeat(200))
  log('超长名字拒绝', !addLongRes.data?.success, '(应返回失败)')

  // 查询分数
  const scoreRes = await callApi(page, 'eaa.score', 'CDP_Electron_A')
  log('查询学生分数', scoreRes.ok && scoreRes.data?.success !== false, scoreRes.error || '')

  // 删除测试学生
  const delRes = await callApi(page, 'eaa.deleteStudent', 'CDP_Electron_A', 'test cleanup')
  log('删除测试学生A', delRes.ok && delRes.data?.success !== false, delRes.error || '')
  ...
  const uniqueClassId = `CDP-TEST-${Date.now()}`
  ...
  log('空参数创建拒绝', !createEmptyRes.data?.success, '(应返回失败)')
  ...
  // 先添加测试学生
  await callApi(page, 'eaa.addStudent', 'CDP_Event_Test')

  // 添加加分事件
  const addEventRes = await callApi(page, 'eaa.addEvent', {
    studentName: 'CDP_Event_Test',
    reasonCode: 'CLASS_MONITOR',
    delta: 10,
    note: 'CDP测试加分',
  })
  log('添加加分事件(+10)', addEventRes.ok && addEventRes.data?.success !== false, addEventRes.error || '')

  // 添加扣分事件
  const deductRes = await callApi(page, 'eaa.addEvent', {
    studentName: 'CDP_Event_Test',
    reasonCode: 'LATE',
    delta: -2,
    note: 'CDP测试扣分',
  })
  log('添加扣分事件(-2)', deductRes.ok && deductRes.data?.success !== false, deductRes.error || '')

  // dryRun 模式 - EAA CLI --dry-run 应返回 exitCode 0
  const dryRunRes = await callApi(page, 'eaa.addEvent', {
    studentName: 'CDP_Event_Test',
    reasonCode: 'CLASS_MONITOR',
    delta: 100,
    note: 'dryRun测试',
  ...
  log('dryRun模式不写入', dryRunRes.ok && dryRunRes.data?.success !== false, dryRunRes.error || dryRunRes.data?.error || `exitCode=${dryRunRes.data?.exitCode} stderr=${dryRunRes.data?.stderr?.slice(0, 100)}`)

  // 验证分数
  const eventScoreRes = await callApi(page, 'eaa.score', 'CDP_Event_Test')
  log('验证分数正确', eventScoreRes.ok && eventScoreRes.data?.success !== false, `score=${eventScoreRes.data?.data?.score}`)

  // 不存在学生加事件
  const ghostEventRes = await callApi(page, 'eaa.addEvent', {
  ...
  // 查询历史
  const historyRes = await callApi(page, 'eaa.history', 'CDP_Event_Test')
  log('查询学生历史', historyRes.ok && historyRes.data?.success !== false, historyRes.error || '')

  // 清理
  await callApi(page, 'eaa.deleteStudent', 'CDP_Event_Test', 'cleanup')
  ...
  const agentCount = agentListRes.data?.length || 0
  ...
  // 获取 SOUL.md
  const soulRes = await callApi(page, 'agent.getSoul', 'main')
  log('获取SOUL.md', soulRes.ok, soulRes.error || '')

  // 获取 AGENTS.md
  const rulesRes = await callApi(page, 'agent.getRules', 'main')
  ...
  // 获取历史
  const agentHistoryRes = await callApi(page, 'agent.getHistory', 'main')
  ...
  const providersRes = await callApi(page, 'ai.listProviders')
  log('AI供应商列表', providersRes.ok, providersRes.error || '')
  const providerCount = Array.isArray(providersRes.data) ? providersRes.data.length : 0
  ...
  // 保存技能
  const skillSaveRes = await callApi(page, 'skill.save', 'cdp-test-skill', '# CDP Test Skill\nThis is a test.')
  log('保存测试技能', skillSaveRes.ok && skillSaveRes.data?.success !== false, skillSaveRes.error || '')

  // 读取技能
  const skillGetRes = await callApi(page, 'skill.get', 'cdp-test-skill')
  log('读取测试技能', skillGetRes.ok, skillGetRes.error || '')

  // 删除技能
  const skillDelRes = await callApi(page, 'skill.delete', 'cdp-test-skill')
  ...
  // 添加测试任务 - cron:add 需要 expression 字段（非 schedule）
  ...
  log('非法主题值拒绝', !settingsBadRes.data?.success, '(应返回失败)')
  ...
  const chatSessionsRes = await callApi(page, 'chat.listSessions')
  ...
  const exportFormatsRes = await callApi(page, 'eaa.exportFormats')
  ...
  for (let i = 0; i < 10; i++) {
    concurrentPromises.push(callApi(page, 'eaa.listStudents'))
  }
  ...
  // ===== 18. 学生档案测试 =====
  console.log('\n━━━ 18. 学生档案测试 ━━━')
  await callApi(page, 'eaa.addStudent', 'CDP_Profile_Test')
  const profileSetRes = await callApi(page, 'profile.set', 'CDP_Profile_Test', {
    notes: 'CDP测试档案',
    tags: ['test', 'cdp'],
  })
  log('设置学生档案', profileSetRes.ok && profileSetRes.data?.success !== false, profileSetRes.error || '')

  const profileGetRes = await callApi(page, 'profile.get', 'CDP_Profile_Test')
  log('读取学生档案', profileGetRes.ok && profileGetRes.data?.success !== false, profileGetRes.error || '')

  await callApi(page, 'eaa.deleteStudent', 'CDP_Profile_Test', 'cleanup')
